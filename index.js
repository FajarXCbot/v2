...
/*=====================================================[ CONNECTING  ]==============================================================*/                  	    
            ...
            nayla.version = [2, 2143, 3] 
	        nayla.logger.level = 'warn'
	        console.log(banner.string)
        	nayla.on('qr', () => {
     		console.log(color('[','white'), color('!','red'), color(']','white'), color('SUBSCRIBE YOUTUBE NIMEZZ BOTZ'))
	        })
	        fs.existsSync('./self-bot.json') && nayla.loadAuthInfo('./self-bot.json')
	        nayla.on('connecting', () => {
		    start('2', 'Subscribe YouTube Nimezz botz...')		   
        	})
	        nayla.on('open', () => {
	    	success('2', 'Connected')
	    	setTimeout( () => {
	    	console.log(color(`[❗] SUBSCRIBE YOUTUBE Poonix Ganz`, 'aqua'))
	    	}, 1000)    		    	     	
         	})         	
         	nayla.sendMessage(`${setting.ownerNumber}@s.whatsapp.net`, `Successfully connected with your WhatsApp, don't forget to subscribe https://youtube.com/channel/UCg-iDt2kh9W-8m8K425E-pAMoRYlA for more update`, MessageType.text)
...
/*=======================================================[ WELCOME ]==============================================================*/                  	    
        ...
        try {
            ...
            if (chat.action == 'remove') {
                ...
                ini_out = `Bye bye BEBAN GRUP @${mem.split('@')[0]} balik lagi GW GEDIK`
...
/*=====================================================[ API VIDEFIKRI ]==============================================================*/                  	                 
			...
			only: {
				group: '```Fitur Ini Cuma Bisa Di Grup Tod! ```',
				...
				ownerB: '```Fitur Ini Khusus OWNER BOT! ```',
				premium: '```Fitur Ini Khusus Member Premium! ```',
				userB: `Hai ${pushname} Kamu Belum Ada Di Database, Sebelum Menggunakan fitur ketik ${prefix}verify`,
				admin: '```Lu Bukan Admin! ```',
				Badmin: '```Bot Bukan Admin Bitch. ```',
				}
			}  
//HARI & BULAN
var date = new Date();
...
//+++++++++++TIME+++++++++++//
 ...
 //+++++++++++UCAPAN+++++++++++//
            ...
            */const vnayla = 'BEGIN:VCARD\n' 
            ...
            + `FN: YOGA\n` 
            + `ORG: AUTHOR;\n`
            + `TEL;type=CELL;type=VOICE;waid=6282376320967:+6282376320967\n`
            + 'END:VCARD' 
...
*/                                        
		...
		case 'daftar':
					...
					const captionnya = `╭─「 *SUKSES MASUK DATABASE!* 」\n│ \`\`\`Succes Login Ke Database\`\`\` \n│ \`\`\`SN: ${noSeri}\`\`\`\n│\n│\`\`\`On ${wib}\`\`\`\n│\`\`\` Name : ${pushname}\`\`\`\n│\`\`\`Number : wa.me/${sender.split("@")[0]}\`\`\`\n│\`\`\`Silahkan Menggunakan Bot\`\`\`\n│\`\`\`send command ${prefix}menu\`\`\`\n│\`\`\`\n│ Users total : ${_Elite.length} Users\`\`\`\n╰─────────────────────────`
        ...
        case 'menu':
        case 'help':
                   if (!isElite) return reply(`Hai ${pushname} Kamu Belum Ada Di Database, Daftar Yuk! Ketik ${prefix}verify`)
	                numbernye  ='0'
					...
					menunye = `╭─❒ *INFO BOT* 
│❒ 𝖭𝖺𝗆𝖺 : ${botname}
│❒ Author : Naylaaa Chann:v
│❒ Recode : FajarKyoo (Case) 
│❒ Recode² : Yoga Ganz (Nimezz botz)
│❒ Recode³ : Poonix Ganz
│❒ Server : Local-Host 0964 
...
/*====================================================[ CASE BY NAYLA ]==============================================================*/                    	                                          
      ...
      case 'qrcode':  
                   ...
                   nayla.sendMessage(from, anu, image, {caption: `SELESAI!\n\nFollow TikTok : @imyourexhaha`, quoted: nay1})
                   break
      case 'resepmasakan':  
                   if (!isElite) return reply(mess.only.userB)
			       if (isLimit(sender)) return
      ...
      case 'barcode':  
                   ...
                   break
       case 'ping':
                   reply("Ya kak ada yang bisa Cbot F Bantu?") 
     case 'cersex':  
                   if (!isElite) return reply(mess.only.userB)
			       if (isLimit(sender)) return
			       await limitAdd(sender)	
...
/*====================================================[ CASE BY NAYLA ]==============================================================*/                    	                                          
       ...
       case 'author':  
	    ...
	    case 's':  	
					...
					.on('end', function () {
					console.log('SELESAI JANGAN LUPA SUBSCRIBE YT Poonix Ganz')
...
/*====================================================[ CASE BY NAYLA ]==============================================================*/                    	 
		...
		case 'request':  
                    ...
                    nayla.sendMessage(`6285161632310@s.whatsapp.net`, optionsp, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "caption": `*NIHHH WOIII REQUEST*`} } } })                                    
...
/*====================================================[ CASE BY NAYLA ]==============================================================*/                    	 
	    ...
	    case 'deteksiumur':  
     	...
     	case 'demote':  
			        ...
			        await limitAdd(sender)
					if (!isGroup) return reply('GRUB ONLY')
					if (!isGroupAdmins) return reply('LU ADMIN??, KALO BUKAN YAA UDAH')
					if (!isBotGroupAdmins) return reply('BOT BUKAN ADMIN TOLL')
		...
		case 'pesan':
					...
					reply('PESAN SUDAH TERKIRIM MONYT GAK USAH SPAM')
					break
		case 'tebakkimia': 
                    if (!isElite) return reply(mess.only.userB)
			        if (isLimit(sender)) return
...
/*====================================================[ CASE BY NAYLA ]==============================================================*/                    	 				    
                    ...
                    anu1 = `Maaf Bro! *${pushname}* ${notc1} Tapi Fitur *${prefix}${command}*Gak Ada Di ${prefix} menu *`
